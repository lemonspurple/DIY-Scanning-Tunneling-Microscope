...
This repository exists to consolidate the scattered knowledge and parts needed as well as to preserve the documentation and everything needed to build your own. 
...
* Female Pin Headers to mount for ESP32, DAC 2 and ADC 8
...
### PCB
* **Controller Board:** Check PSB folder for stable release or this [repo](https://github.com/PeterDirnhofer/KiCad_RTM_Adapterboard_v3_0) for the latest version.
* **Tunnelling Amp Board:** Check PSB folder for stable release or this [repo](https://github.com/PeterDirnhofer/Tunnelling-Amp-20) for the latest version.
...
While most parts are commonly available off-the-self, there still are some outliers that are worth ordering first, as their availability might vary.

### Aquisition

- **Mechanics:** Parts such as the KC1-T/M mount from [Thorlabs](https://www.thorlabs.com/) have to be ordered directly from the manufacturer. You'll likely have to reach out to them first (via eMail or phone), as they're working mostly with institutions it seems. Keep in mind that you'll also have to drill holes using a standing drill, followed by cutting threads for the screws. You can find the tools you'd need in maker- and hackerspaces often.

- **PCB:** The board for the controllerboard and transimpedance amplifier have to be printed by a PCB manufacturer. The gerber files for that are freely available for download, but the production process usually takes ~10 business days, so it's worth planning ahead. The ones we're using were provided by the Uni Regensburg.
...
- **Conductive silver lacquer:** When you're soldering something onto the piezo disk, there is a high chance that you're going to ruin the component through heating. Instead of soldering the pins to the disk directly, you can alternatively super glue them on top. To restore conductivity, silver lacquer, which is often used among model makers, can be painted over the parts afterwards.
...
- **Redundancy:** Components that break often are marked with a 𐤲 in the bill of materials. Especially the piezo disks and transimpendance components are worth considering buying in bigger quantities, be it for the peace of mind that you don't have to nail it on the first attempt.
...
- **Multimeter verification:** Speaking about everything piezo disk, controller and transimpendance board, it is worth verifiying if all parts connect properly after soldering. Especially the gluing of the pins to the disk together with the copper wire is prone to shorts or errors and has to be checked.
...
<div align="center"><sup>(Fig 1. The piezo buzzer is being held by the third hand with pins already glued to the quadrants and silver laquer applied to restore conductivity. Copper wire connects each pin to the DAC elements)</sup></div>

With an exacto knife/scalpel and ruler you're going to divide the brighter middle part of the piezo disk into quarters (the ceramic). After that, apply some super glue to the gold pins and attach one on each quarter. The super glue will take some time to dry. After that, remove insulation from the copper wires with a lighter and solder them to the top of the pins as seen in the picture. You can then use conductive silver laquer to restore conductivity between pin and the quarter of the piezo disk in case it has been interrupted by the glue.  
After that, solder a fifth copper wire to the outer ring for grounding.

>[!WARNING]
>Verifiy with the multimeter that the quadrants are disconnected from each other before executing any of the other steps mentioned. It is worth double checking the copper wire and silver laquer connection to the piezo disk as well.

After the construction has dried, flip it around and prepare the two-part epoxy adhesive. Try to glue the fifth pin as precise as you can on the middle of the other side of the piezo disk. **Compared to the other five connections, this pin is not allowed to be connected to the circuit and has to be isolated by the glue. Verify with your multimeter that it is not connected to the piezo disk.**
You can make this task a bit easier by attaching the pin to a piece of plastic and gluing this to the piezo buzzer to guarantee that there is no electric connection between the last pin and the disk.
...
There really isn't much you can do wrong when placing these parts as long as you follow the instructions on the left side. For example, R309 stands for resistor number 309. It has to have 220 Ohm. C203 is a capacitor that requires 4nF (that's nanofarad). You can double check both values with most multimeters, if you're extra paranoid.
...
> [!NOTE]
> LED's might be missplaced (positive/negative swapped). Solder them on last after verifiying that they are correct. We're working on verficiation / fixes ourselves on that matter.
...
If you managed the schematics of the controller you will do with these just fine. It is really worth trainign SMD soldering beforehand though.
C1,C2 get a 47nF capacitor and C3,C4 1µF. C5 and C6 require 4,7 µF/35V **and are polarized, hence you have to pay attention to positive/negative.** C5, and C6 **really** have to specifically work with 35V. I've used the TAJ 6032 4,7/35.
...
The lenseholder requires you to drill 4 holes and cut threads for the screws. This can be done easiest with a standing drill. I used a hammer on a center punch on the positions for the screws to give the drill some extra grip and went all out on the machine oil. The holder is made out of aluminium, which is easier to cut through than it sounds, but it's worth going very slowly.
...
>[!WARNING]
>The lenseholder has intricate mechanics inside it's plate. Turn it around (See Fig 7.) and be very mindful to not accidentally drill through them.
...
> [!NOTE]
> As you can see in Fig. 9, the bottom of the measuring element glued to the piezo element is pointing downwards. It is worth considering how you incorporate this construction into the vibration isolation, as you will be soldering new tips to the pin quite frequently.
...
- B) The piezo element connects with the controller board as shown above. You do not wire them directly to the piezo element, but rather through the copper wires (Fig. 1, Fig. 9) to make sure they can move easily. Z+X and Z-X have to be neighbours and the same goes for Z+Y and Z-Y (In other words: If the piezo would be an analogue clock, Z+X and Z-X could be at 2 and 5, but not 2 and 7). 
You can also see in Fig. 9 that I soldered the orange ground cable (in Fig. 11 it is purple) to the plates that hold the piezo. Verify with the multimeter that there is not shorting and that ground connects to the outer rim of the piezo element.
I have fixated the cables using Fixogum (light, nondestructive, easy to remove glue).

- C) This is the bottom side of the piezo element. The tip simply connects to the IN plate of the transimpendance amplifier. Keep this as short as possible to reduce interference.
...
- E) USB Connector is wired to each of the respective ports. If the COM port shows up, but you can't connect, it is very likely because you confused RX and TX. This USB cable is a requirement because it supports a baud rate for 40k+, which is needed for the monitoring of the data. The micro USB port of the ESP32 caps at around 10k and wont work.

![The brass plate holds the sample](/images/pad_connected_to_battery.jpg)
<div align="center"><sup>(Fig 12. The pad holds the sample and is glued using Fixogum to a microscope glass for added support and weight.)</sup></div>
  
  

- F) The brass plate holds the sample you're going to measure. You apply conductive adhesive pads to it that will hold the sample (These are used to fix remotes and can be obtained for cheap). You glue the plate also on the microscope glass plates using Fixogum for better stability. Fixogum is a super light glue that can be easily removed. Double check later if your sample is connected properly, as the conductive pads quality varies.
...
<div align="center"><sup>(Fig 14. tips cut from silver wire under a microscope. The round results were made by creating break points with a lighter)</sup></div>
Silver wire is sufficient to produce tips (the slimmer the better), however, you have to produce a tip that is so sharp that it's only a single atom sharp at best. This is not as hard as it sounds, but also not as easy. You can try holding it under a lighter to create a breakpoint that can be pulled appart with some tongs. Personally, I achieved the best result by practicing to cut it at a 30° angle and inspecting it under an optical microscope to warrant that it's a good result.

You don't have to hurry, but you also don't have all the time in the word, as silver, especially if there are only a single or only few freaking atoms at the top, will corrode very fast (perhaps an hour at best). You can circumvent this by using platin iridium wire, however, it's very expensive and I burned through some meters before I produced good results.
...
If you're using the Espressif Flashing Tool you want to open the .exe you downloaded and select **ESP32 as ChipType** and **WorkMode Develop**.

![The Flashing menu](/images/firmware.png)
...
2. Create a new virtual environment and activate it  by copying the following commands.
...
3. Install the required dependencies (The virtual environment has to be activated through step 2 for this to work and is indicated by a (.venv) in front of the line input)
...
It can happen that VSCode prompts you to install Python extensions for it to run properly. (Which you should do)
...
7. A GUI should now be open. You can try Tools > Sinus to start a quick test. If you connected everything properly, you should hear your microscope make a subtle sound. You can use the other simulations to check, if you get basic functionality or errors to verify if you set up everything properly.

8. You can now add your sample to the brass plate and solder on a tip to start your first measurements. You can use the tunnel tool with the standard parameters to get a feeling for what is basic interference and when you turned the screws of the lenseholder too much and potentially made contact with your sample.
